...
def WFVW2(N, d, L, H, x, ww, method):
    ...
    for i in range(1, (N + 1)):
        cosh2 = math.cosh(k * (d + yo))
        cosh3 = math.cosh(k * (2 * d + yo))
        cosh4 = math.cosh(k * yo)
        sinh2 = math.sinh(k * (d + yo))
        sinh3 = math.sinh(k * (2 * d + yo))
        sinh4 = math.sinh(k * yo)
        ...
        if method == 0: #Miche-Rundgren (method = 0)
            yc[j, 1] = yo + (H / 2) * (1 + x) * (sinh2 / sinh1) + (math.pi * H / 4) * (H / L) * (sinh2 / sinh1) * (cosh2 / sinh1) * ((1 + x)**2 * theta1 + (1 - x)**2 * theta2)
            yt[j, 1] = yo - (H / 2) * (1 + x) * (sinh2 / sinh1) + (math.pi * H / 4) * (H / L) * (sinh2 / sinh1) * (cosh2 / sinh1) * ((1 + x)**2 * theta1 + (1 - x)**2 * theta2)
            ...
            yc[j, 1] = yo + H * (sinh2 / sinh1) + math.pi * H * (H / L) * (sinh2 / sinh1) * (cosh2 / sinh1) #vertical elevation when crest at wall
            yt[j, 1] = yo - H * (sinh2 / sinh1) + math.pi * H * (H / L) * (sinh2 / sinh1) * (cosh2 / sinh1) #vertical elevation when trough at wall
